...
def signup():
    ...
    if (
        "zip_code" not in data["address"]
        or "state" not in data["address"]
        or "city" not in data["address"]
        or "number" not in data["address"]
    ):
...
def update_client(id):
    ...
    if "full_name" not in data:
        ...
        "email" in data
        and data["email"] != client.email
        and Client.query.filter_by(email=data["email"]).first()
    ):
